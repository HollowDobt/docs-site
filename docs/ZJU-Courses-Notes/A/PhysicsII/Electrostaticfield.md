...
!!! note
	高中阶段学习的库仑定律为我们理解静电场奠定了基础: 在真空中, 一个鼓励点电荷 $Q$ 产生的电场强度 $\overrightarrow E$  可由 $\overrightarrow E =k\frac{Q}{r^2}\widehat r$ (其中 $\widehat r$ 表示单位方向向量)确定. 这一表达式之所以成立, 核心在于点电荷形成的电场线具有高度的球对称性, 即所有电场线都垂直穿过以点电荷为球心的任意球面. 正是基于这种对称性, 我们尝试去量化电场穿过的一个假想曲面(我们称之为高斯面)的"通量".
	
	若构造一个半径为 $r$ 的球形高斯面, 类比于磁场, 将电场强度 $\overrightarrow E$ 与球面面积 $4\pi r^2$ 相乘, 我们可以得到电通量: $\Phi_E=\oiint_\Sigma \overrightarrow E\cdot \mathrm d \overrightarrow S=|\overrightarrow E|\cdot 4\pi r^2$ ($\overrightarrow E$ 表示面积矢量, 方向为**外法线方向**. 外法线的方向就是指封闭曲面中从曲面内部指向外部并且垂直于曲面(微元)的方向). 带入 $\overrightarrow E$ 的库仑定律表达式, 发现恰好有: $\Phi_E\propto{|\overrightarrow E|}$. 我们通过实验发现, 这一正比关系对应的系数只与被包围电荷所处的环境相关(电荷周围的环境也不会影响), 我们便定义其为**介电常数**, 原始表达式即可变为: $\Phi_E=\frac{Q}{\varepsilon}$.
	
	在学习了微积分中的第二类曲面积分后, 我们发现电场通量与包围电荷之间的关系具有更深刻的普适性, 它不依赖于电荷分布的形状, 也不依赖于我们所选取的高斯面的形状. 只要电场是静电场, 无论高斯面是一个球体, 立方体还是任何连续封闭的曲面 $\Sigma$, 电通量的总和都仅仅取决于该曲面内部所包围的净电荷 $Q$. 这便是**高斯定理**.

## 利用高斯定理求解电场

在满足第二类曲面积分要求的前提下, 导出高斯定理原型:

$$
\oiint_{\Sigma}\overrightarrow E(s)\ \cdot\ \mathrm d\vec s =\frac{Q}{\varepsilon}
$$

其中, $\overrightarrow E(\vec s)$ 表示对应曲面微元处的电场强度, $\Sigma$ 表示封闭的空间区域(或者边缘无限远的空间区域), $\vec s$ 表示某一微小空间区域的面积矢量, $Q$ 表示该封闭空间区域内包围的净电荷量, $\varepsilon$ 表示介电常数.
