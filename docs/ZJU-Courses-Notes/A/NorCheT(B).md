...
!!! abstract
	这部分内容建议提前两到三天复习, 考前尽量熟悉并理解大部分实验的操作流程与核心原理. 尤其是从我们这届(2025 年春夏)起, 普化乙实验不再能仅凭死记硬背拿高分. 考试中部分填空题设问相当细碎, 因此需要尽可能把握更多操作细节. 
	
	另外, 如上面所说, 考试中细节颇多, 只看这个笔记是无法保证能拿到满分的(呜呜呜).

## 酸碱互滴
---

- 溶液配置的方法有三种, 直接水溶法, 介质水溶法(也就是掺杂了酸碱或特定有机溶剂的溶液), 稀释法.
- $NaOH$ 标准液容易吸收空气中的 $CO_2$ 生成 $Na_2CO_3$, 会改变有效浓度并带来系统误差: 实验中应尽量隔绝 $CO_2$ (加塞, 用碱石灰/碱液吸收等). $Ba(OH)_2$ 因生成难溶 $BaCO_3$, 可用于**吸收/去除** $CO_2$ (但溶液浓度仍会随吸收而变化, 使用前同样需要标定).
...
!!! tip
    上面这一点如何保证? 实际上就是指的**称量仪器的精度足够大**. 例如, 如果你要称取 $0.1000 g$ 的物质, 但是天平精度只有 $\pm0.001 g$, 那么相对误差就是
...
!!! tip
    这里说的误差值其实**就是指前面说的精度值**. 精度形式应当形如 $\pm0.001 g$ 只有一位有效数字, 没有诸如 $\pm0.00128371 g$ 的形式.

- 浓盐酸和固体氢氧化钠都不是基准物质,因此配置滴定溶液的时候使用间接配制法.

- 标定 $NaOH$ 的(用于与 $NaOH$ 反应的基准)物质通常选择草酸/**邻苯二甲酸氢钾**，反应用酚酞做指示剂.

(下述(常见非基准物质对应基准物质与相应指示剂)表格相当复杂, 可以在考试之前记一下. 其他不建议浪费时间在这上面)

| 标液           | 基准物质                    | 指示剂            |
| ------------ | ----------------------- | -------------- |
| $HCl$        | 碳酸钠($Na_2CO_3$)         | 甲基红, 甲基橙       |
| $NaOH$       | 邻苯二甲酸氢钾, 草酸             | 酚酞             |
| $EDTA$       | $CaCO_3, ZnO$, 纯锌($Zn$) | $EBT, K-B, XO$ |
| $KMnO_4$     | $Na_2C_3O_4$            | $KMnO_4$ 自身    |
| $Na_2S_2O_3$ | $K_2Cr_2O_7,KIO_3$      | 淀粉             |
| $I_2$        | $As_2O_3$               | 淀粉             |
...
!!! tip
    ...
    3, 增加测量次数, 减少偶然误差（主要是**随机误差**）；**过失误差**属于操作失误，应通过规范操作与数据检验避免/剔除，而不是靠“多测几次”去平均掉。
    
    4, 加入空白对照等排除系统误差;

- 精密度与偏差: 类似于数学上的方差和均方差. 注意, **偏差可正可负, 但是相对平均偏差一定是正的(有绝对值)**. 同时平均值在偏差这里也是永远没有"被"字的. 相对平均偏差越小, 数据的可靠性就越好

- 有效数字保留规则.

!!! info
    加减法: 规则最简单, 保留到小数位数最少得数字的一位; 乘除法相对复杂, 最终结果取决于更高等的有效位数; 而指对运算最复杂, 对数一般小数点多所以它所占的位数理应更长一点, 所以取对数之后应该增长, 就是有效数字位数变成小数位数.对数/指数运算: (比较复杂)对数运算后的数的**小数位数**就是对数运算前的数的**有效数字位数**. 指数运算则相反(这一规则的解释详见: [中文维基百科](https://zh.wikipedia.org/wiki/%E6%9C%89%E6%95%88%E6%95%B0%E5%AD%97)). 例如: $\lg(45.6)=1.660$，$10^{1.660}=45.6$. 乘除法: 按**有效数字位数**最少的保留. 例如: $11.45 \times 1.4 = 16$. 加减法: 按小数点后位数最少的保留. 例如: $11.45 + 1.4 = 12.9$. 

- 纸屑等固废不可扔入"废玻璃桶"
- 酸腐蚀急救: **立刻用大量流动清水冲洗至少 15 min**. 皮肤可在充分冲洗后再用稀 $NaHCO_3$ 溶液轻轻处理; **眼部用清水/生理盐水持续冲洗**, 并尽快就医.
- 碱腐蚀急救: **立刻用大量流动清水冲洗至少 15 min**. 皮肤可在充分冲洗后再用稀硼酸/稀醋酸轻轻处理; **眼部同样只用清水/生理盐水持续冲洗**, 并尽快就医.
- 毒气急救: **立即撤离到通风处/室外**, 保持呼吸道通畅, 必要时吸氧并尽快就医.
...
## 电镀铜
---
- **电镀时**(主要与后面的蚀刻对比), 阴极是待镀的电极(钢板), 阳极是作为镀层的金属或惰性金属(一般仍选用镀层金属铜以维持 $Cu^{2+}$ 供给). **如果选择惰性阳极, 通常发生的是水的析氧反应**（总体可写作 $2H_2O\rightarrow O_2+4H^++4e^-$；在碱性条件下也可写作 $4OH^-\rightarrow O_2+2H_2O+4e^-$）。
- **电镀液的选择**直接决定了电镀的质量.
...
!!! info
    铜片作阳极, 不锈钢片作阴极, 板间距维持固定(1~2 cm, 注意绝对不能短路), 电镀条件有:(1), $0.12 A$ 电流, $10 min$; (2), $0.06 A$ 电流, $20 min$.

- 电化学刻蚀操作

!!! info
    用修正液画好图案, 交换原来连接不锈钢片与铜板的电源线(也就是电流方向逆向), 改原来电镀液为刻蚀液, 电流控制在 $0.4\sim0.5 A$, 时间控制在 $8\sim10 min$. 蚀刻完毕后使用**乙酸乙酯**除去修正液.

## 解热镇痛药阿司匹林的制备与提纯
---
- 主反应与副反应
- 实验步骤回顾: 称量水杨酸和乙酸酐, 搭建好实验装置, 在水浴加热、**少量浓磷酸（或少量浓硫酸）催化**条件下反应, 结束后冷水浴冷却结晶(可以摩擦内壁促进结晶), 之后抽滤得到固体粗产品; 将固体粗产品溶于$NaHCO_3$溶液中, 再次抽滤, 得到去除了聚合物的滤液; 将滤液与盐酸混合反应, 再次抽滤得到固体乙酰水杨酸.
- 可以使用少量冷水洗涤得到的粗/精产品 1, 2 次.

!!! info
    阿司匹林合成反应. 主反应(酰化反应, 酰化的位点是水杨酸的**酚羟基**）：
	
    $$
    C_7H_6O_3\,(水杨酸)+ (CH_3CO)_2O\,(乙酸酐)
    \longrightarrow C_9H_8O_4\,(乙酰水杨酸)+ CH_3COOH\,(乙酸)
    $$
	
    条件要点：尽量**无水**（乙酸酐遇水会水解）、水浴加热、酸催化（常用少量浓磷酸/浓硫酸）。
	
    主要副反应（实验中最常见的“真的会影响产率”的那种）：
	
    $$
    (CH_3CO)_2O + H_2O \longrightarrow 2CH_3COOH
    $$
	
    以及产物/原料在水中可能发生一定程度水解或带入未反应水杨酸（因此后续通常还需要重结晶进一步提纯）。

- 乙酰水杨酸化学提纯: 用 $NaHCO_3$ 将粗产物中的酸性组分(乙酰水杨酸, 可能残留的水杨酸)转化为可溶的钠盐进入溶液; 过滤去除**不溶性杂质**. 随后用盐酸酸化, 使乙酰水杨酸重新析出并抽滤得到固体. 注意: 此步骤**不一定能完全去除水杨酸**, 更彻底的提纯依赖后续重结晶/洗涤.
...
!!! info
    $$
    Mg(s) + 2H^+(aq.)=Mg^{2+}(aq.)+H_2\uparrow
    $$
!!! info
    道尔顿分压定律: 利用理想气体状态方程: $pV=nRT$(注意单位: $Pa\cdot m^3\cdot K^{-1}\cdot mol^{-1}$, 或者写作 $J\cdot K^{-1}\cdot mol^{-1}$. 实际上记住后面这个就可以了, 前面那个使用气体功的思路很容易想到. **但是计算的时候更常见的是前面一种**. 注意另外给的单位是不是 **mL**, mL 的话与 $m^3$ 差了 9 个数量级.记住两个的单位都是国际标准单位, 没有任何特殊的多余的符号(像$k, c, m$之类的)), 我们很容易推出, 在外界大气压与管内气压平衡时
    
    $$
    R=\frac{p_{H_2}V_{H_2}}{n_{H_2}T}=\frac{(p_{大气压}-p_{水蒸气})\Delta V}{\frac{m_{Mg}}{M_{Mg}}(t+273.15)}
    $$
    
    其中 $\Delta V$ 是量气管读出的**气体体积变化量**(反应前后读数差). 若量气管与水准瓶液面未严格齐平, 还需额外考虑液柱高度差带来的压强修正; 实验操作上通过调节水准瓶让两边液面尽量齐平来消除该项.
...
- 为了保证尽量不漏气, 在反应过程中水准瓶应该随着量气管液面的变化而变化, 保证量气管和水准瓶的液面尽可能维持在同一水平面.
- 误差分析的时候考虑一层即可, 不必多想.
...
- 三草酸合铁(III)酸钾的光敏性应用（光还原 → 蓝晒）：光照下 $[Fe^{III}(C_2O_4)_3]^{3-}$ 被还原为 $Fe^{II}$（同时草酸根被氧化并放出 $CO_2$ 等），生成的 $Fe^{II}$ 随后与铁氰化钾中的 $[Fe^{III}(CN)_6]^{3-}$ 反应生成普鲁士蓝沉淀（蓝色）。示意写法：$[Fe^{III}(C_2O_4)_3]^{3-} \rightarrow\,(h\nu) [Fe^{II}(C_2O_4)_2]^{2-}+CO_2+\cdots$；$Fe^{II} + [Fe^{III}(CN)_6]^{3-} \rightarrow Fe^{III}[Fe^{II}(CN)_6] \downarrow$
- 结晶析出方法: 高温蒸发, 蒸发浓缩冷却结晶, 改变溶剂, 化学反应沉淀.
- 蓝晒法: 也称为**铁氰酸盐印相法**, 这是第一种非银盐摄影工艺(感光液由三草酸合铁酸钾溶液和铁氰化钾溶液 1:1 体积组成).
- 实验步骤回顾: 首先称量草酸钾, 加水溶解, 加热接近沸腾, 边搅拌边加入($Fe$ **离子都是这个德性**)氯化铁溶液, 加完之后沸腾一段时间, 然后稍微冷却, 再转入冰水浴冷却结晶, 抽滤得到滤饼（晶体）作为粗产品. 而后我们进行**重结晶**操作, 使用热水溶解, 趁热过滤, 再次冷却结晶, 得到最终的精产品.
...
!!! info
    ...
    $$
    (-)Ag|AgCl, 内参比溶液|玻璃膜|试液||KCl(饱和 aq.), Hg_2Cl_2|Hg(+)
    $$
    ...
    注意上述等式并不完全正确. 实际上测定的时候应当考虑活度系数 $\alpha$, 应当使用活度定义：$pH=-\lg a_{H^+}=-\lg(\gamma\,[H^+])$. 不过考虑到基本都是适当浓度(不是很浓不用考虑分子不电离, 不是很稀不用考虑水的电离)的稀溶液, 所以一般认为活度系数就是 1.
...
- pH 计使用前需要预热 $10\sim20 min$.
- pH 计测定的溶液 pH 范围不宜超过 $1\sim10$, 超出范围后误差极大.
...
!!! info
    ...
    如果原料是**脂肪酸**（如月桂酸、肉豆蔻酸、硬脂酸），本质是酸碱中和：

    $$
    RCOOH + NaOH \longrightarrow RCOONa + H_2O
    $$

    如果原料是**油脂（甘油三酯）**（如蓖麻油），则为真正意义上的皂化（水解）：
    ...
    其中$R$可以是: $C_{11}H_{23}$(构成**月桂酸**, 又称为**十二酸**. 联想: 一年有 12 个月), $C_{13}H_{27}$(构成**肉豆蔻酸**, 又称为**十四酸**. 联想: 女子十四岁称豆蔻年华), $C_{17}H_{35}$(构成**硬脂酸**, 又称为**十八酸**. 联想: 十八岁的硬汉). 当然也可能考到蓖麻酸(ricinoleic acid), 它的分子式为 $C_{18}H_{34}O_3$; 在写 $R-COOH$ 的“$R$”时可以写成 $C_{17}H_{33}O$(因为 $R$ 里包含一个羟基和一个不饱和键), 错误的讲就是把两个氢变成了一个氧("维持化合价"))再打个括号标个"蓖麻酸".
...
- 转移蓖麻油时如果有残余可以使用 $2\sim5 mL$ 的乙醇清洗.
...
!!! info
    ...
    **STEP 4 鉴定** 计算比移值($R_f$ 值). 一般要求 $R_f$ 值在 0.2~0.8 之间, 同时 $R_f$ 值之间的差值大于 0.05, **以防斑点重合**. 待到溶剂挥发干净后再在紫外灯下观察, 在 254 nm 紫外灯下，硅胶 $GF_{254}$ 板通常表现为荧光背景上的**暗斑（吸收/淬灭）**。 用尺子记录距离, 计算 $R_f$ 值, 相互对比确定组分含量.
...
- 可以自由落体敲击毛细管让样品装填紧实.
- 乙酰水杨酸是弱酸，$pK_a$（25 °C）约为 **3.5**，可用 $NaOH$ 标准液进行酸碱滴定测定含量。
...
- 本实验采用的敏感膜: 主要由 $LaF_3$(用来提供氟离子的迁移通道)构成. 但是掺杂的有 $EuF_2$(用于提高晶体中 $F^-$ 的迁移率（$Eu^{2+}$ 取代 $La^{3+}$ 会引入 $F^-$ 空位以维持电中性，从而提升离子导电性）). 是氟离子的选择电极, 可以相应的测定氟离子的浓度.
- 膜电极: 仅针对特定离子有选择性响应, 构成离子选择性电极. 其最关键的原件是**选择膜**(敏感膜本质上也是一种选择膜). 能够产生电位差是因为膜内外侧的离子活度不同(注意区分活度系数和活度, 活度就是活度系数乘以相应的离子浓度).

!!! info
    离子选择电极主要由内参比溶液(维持稳定的电极电势), 内参比电极(用于测定内参比溶液的电极电势), 敏感膜(用于形成电位差)组成. 同时应当注意到的一点是, 没有任何元件直接测定外部的电极电势, 本质上是先测定内参比溶液的电极电势, 加上敏感膜测定的电极电势得到的外部溶液的电极电势. 这也意味着, **我们的内参比不能替代参比电极**.
    
    这里给出氟离子选择电极的详细结构, 内参比电极为 $Ag/AgCl$ 电极, 内参比溶液为 $NaF/NaCl$ 溶液(氟离子控制敏感膜内表面电位, 氯离子用以固定内参比电极), 选择膜(敏感膜)为掺有 $EuF_2$ 的 $LaF_3$ 单晶切片.

!!! info
    氟离子选择电极的使用规范: 电极需要在 pH 环境为 5~7 的范围内使用. pH 过高, 溶液中的 $OH^-$ 会与 $F^-$ 做出类似的与氟化镧晶体进行 $F^-$ 交换的行为; pH 过低时, 溶液中的 $H^+$ 会与 $F^-$ 结合形成 $HF$ 或 $HF_2^-$. $HF$ 与 $HF_2^-$ 的生成都会降低溶液中“游离 $F^-$”活度, 通常会使电极读数**偏低**.
...
- 计算氟含量的原理: 标准曲线法和标准加入法. 下面辨析对比优缺点.

!!! info
    标准曲线法: 
    (1) **优点**: 使用一条标准曲线即可完成对多个试样的定量测定, 操作简单;
    (2) **缺点**: 不能排除复杂体系对于测定的干扰.
	
	标准加入法:
    (1) **优点**: 可以有效的消除复杂体系下与标准溶液组成不一致等问题对于测定的干扰, 保证测定结果的准确性和稳定性.
    (2) **缺点**: 每份样品都要测两次, 工作量大, 计算公式复杂.

!!! tip
    标准加入法原理: 根据能斯特方程 $E=K-\frac{2.303RT}{nF}\lg{\gamma c}$, 我们将设样品的体积为 $V_x$, 样品的氟离子浓度为 $c_x$, 加入的标准样品体积为 $V_s$, 浓度为 $c_s$, 可以做出以下推导(已知氟离子参与电极反应电荷变化 $n$ 为 1):
...
!!! info
    分光光度法的特点: 1, **灵敏度较大**, 适用于微量组分的测定; 2, **误差较大**, 相对误差达到 2%~5%.

!!! info
    透光率(透光度): 透过光与入射光的强度之比; 吸光度: 入射光与透过光的强度之比的对数值. 使用公式表示为:
