...
!!! abstract
	我在使用 `OpenCV` 的 `C++` 版本配置环境时顺便学习了使用 `CMake` 配置的方法, 这里记录一下<s>方便下次直接抄</s>. 因为我个人最喜欢使用的编辑器是 VSCode, 所以接下来的示例都以 VSCode 上整体配置过程效果为例进行说明.
...
当然, 很多时候我们会遇到第三方包只有源代码而没有对应的二进制包, 或者编译好的第三方包版本过于老旧的问题. 这个时候的解决方法就是从源代码编译安装, 具体内容待我在树莓派这一 `Linux` 平台上测试过后再续写.
